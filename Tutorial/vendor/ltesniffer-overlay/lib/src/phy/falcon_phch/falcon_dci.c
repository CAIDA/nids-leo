  ...
  if (msg               !=  NULL) {
    ...
    if (format==SRSRAN_DCI_FORMAT0) {
      ...
      if (msg->payload[0]==0) {
        ...
        /*
         * srsRAN's LTE DCI-to-grant helper does not copy the three-bit
         * cyclic-shift-for-DMRS field.  Falcon zeroes the destination grant
         * before this call, so every scheduled PUSCH was incorrectly decoded
         * with n_dmrs=0.  Preserve the parsed DCI value explicitly.
         */
