...
int DCISearch::search() {
  ...
  // A RAR is a one-shot common-search-space message, so histogram-based blind
  // discovery must not be its only path. Search every valid LTE RA-RNTI
  // directly and let the PDSCH transport CRC validate any recovered candidate.
  // This also covers the valid boundary RA-RNTIs 1 and 10.
  if (snr_db > -10.0) {
    for (uint16_t ra_rnti = SRSRAN_RARNTI_START;
         ra_rnti <= SRSRAN_RARNTI_END;
         ++ra_rnti) {
      srsran_dci_dl_t rar_dci[SRSRAN_MAX_DCI_MSG] = {};
      const int nof_rar = srsran_ue_dl_find_dl_dci(
          falcon_ue_dl.q, sf, ue_dl_cfg, ra_rnti, rar_dci);
      for (int i = 0; i < nof_rar; ++i) {
        bool duplicate = false;
        for (const auto& existing : dciCollection.getDLSnifferDCI_DL()) {
          if (existing.rnti == ra_rnti &&
              existing.location.ncce == rar_dci[i].location.ncce &&
              existing.location.L == rar_dci[i].location.L &&
              existing.format == rar_dci[i].format) {
            duplicate = true;
            break;
          }
        }
        ...
        srsran_dci_msg_t rar_msg = {};
        if (srsran_dci_msg_pack_pdsch(&falcon_ue_dl.q->cell,
                                      sf,
                                      &ue_dl_cfg->cfg.dci,
                                      &rar_dci[i],
                                      &rar_msg) != SRSRAN_SUCCESS) {
          continue;
        }
        dci_candidate_t rar_cand = {};
        rar_cand.rnti = ra_rnti;
        rar_cand.dci_msg = rar_msg;
        rar_cand.dci_msg.rnti = ra_rnti;
        rar_cand.search_space_match_result = SEARCH_SPACE_MATCH_RESULT_EXACT;
        dciCollection.addCandidate(
            rar_cand, rar_dci[i].location, UINT32_MAX, sf, &ue_dl_cfg->cfg.dci);
        ret = SRSRAN_SUCCESS;
      }
    }
  }
