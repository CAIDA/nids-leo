...
void ArgManager::defaultArgs(Args& args) {
  ...
  args.file_cell_id = 0;
  args.file_phich_resources = SRSRAN_PHICH_R_1_6;
  args.file_phich_length = SRSRAN_PHICH_NORM;
  args.file_cp = SRSRAN_CP_NORM;
  args.file_wrap = false;
  args.sib2_payload_hex = "";
  args.rf_args = "";
  args.rf_freq = -1.0;
  args.ul_freq = 0;
...
void ArgManager::usage(Args& args, const std::string& prog) {
  ...
  printf("\t-B preload BCCH-DL-SCH SIB2 payload as hexadecimal (offline UL replay)\n");
  ...
  printf("\t-x PHICH resources for input file: 1/6, 1/2, 1, or 2 [Default 1/6]\n");
  printf("\t-X PHICH length for input file: normal or extended [Default normal]\n");
  printf("\t-K cyclic prefix for input file: normal or extended [Default normal]\n");
...
void ArgManager::parseArgs(Args& args, int argc, char **argv) {
  int opt;
  defaultArgs(args);
  while ((opt = getopt(argc, argv, "aABcCDdEfghHilLnpPrRsStTvwWyYqFIuUmOozxXK")) != -1) {
    switch (opt) {
      case 'a':
      ...
      case 'A':
        ...
        break;
      case 'B':
        args.sib2_payload_hex = argv[optind];
        break;
      case 'g':
      ...
      case 'x': {
        ...
        if (value == "1/6") {
          ...
          args.file_phich_resources = SRSRAN_PHICH_R_1_2;
        } else if (value == "1") {
          args.file_phich_resources = SRSRAN_PHICH_R_1;
        } else if (value == "2") {
          args.file_phich_resources = SRSRAN_PHICH_R_2;
        } else {
          fprintf(stderr, "Invalid PHICH resources '%s' (use 1/6, 1/2, 1, or 2)\n", value.c_str());
          exit(-1);
        }
        break;
      }
      case 'X': {
        ...
        if (value == "normal" || value == "Normal") {
          args.file_phich_length = SRSRAN_PHICH_NORM;
        } else if (value == "extended" || value == "Extended") {
          args.file_phich_length = SRSRAN_PHICH_EXT;
        } else {
          fprintf(stderr, "Invalid PHICH length '%s' (use normal or extended)\n", value.c_str());
          exit(-1);
        }
        break;
      }
      case 'K': {
        ...
        if (value == "normal" || value == "Normal") {
          ...
          args.file_cp = SRSRAN_CP_EXT;
        } else {
          fprintf(stderr, "Invalid cyclic prefix '%s' (use normal or extended)\n", value.c_str());
          exit(-1);
        }
        break;
      }
