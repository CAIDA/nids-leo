...
void SubframeWorker::prepare(uint32_t _sf_idx,
                             uint32_t _sfn,
                             uint64_t _input_sf_index,
                             bool updateMetaFormats,
                             srsran_dl_sf_cfg_t _dl_sf)
{
  this->sf_idx = _sf_idx;
  this->sfn = _sfn;
  this->input_sf_index = _input_sf_index;
...
void SubframeWorker::work()
{
  ...
  case UL_MODE:
    ...
    {
      // A PUSCH grant decoded four subframes earlier remains actionable even
      // when blind-search/SNR validation fails in the PUSCH subframe itself.
      // Continue the configured UL pipeline with an empty current-DL
      // collection so stored DCI0/RAR grants are not silently discarded.
...
void SubframeWorker::run_ul_mode(SubframeInfo &subframeInfo, uint32_t tti)
{
  ...
  else
  {
    ...
    if (puschdecoder->get_configed())
    {
      ...
      if (ulsche->get_debug_mode())
      {
        for (const auto& grant : dci_ul)
        {
          if (ulsche->get_rnti() == 0 || grant.rnti == ulsche->get_rnti())
          {
            std::cout << "[UL-GRANT] dci_tti=" << tti
                      << " pusch_tti=" << ((tti + 4) % 10240)
                      << " rnti=" << grant.rnti
                      << " prb=" << grant.ran_ul_grant->n_prb[0]
                      << " len_prb=" << grant.ran_ul_grant->L_prb
                      << " mcs=" << grant.ran_ul_grant->tb.mcs_idx
                      << " tbs=" << grant.ran_ul_grant->tb.tbs
                      << std::endl;
          }
        }
      }
