...
static bool preload_sib2_payload(const std::string& hex, ULSchedule& ulsche)
{
  ...
  for (char character : hex) {
    ...
    if (character >= '0' && character <= '9') {
      ...
      ERROR("Invalid character in preloaded SIB2 payload");
      return false;
    }
    if (high_nibble < 0) {
      high_nibble = nibble;
    } else {
      payload.push_back(static_cast<uint8_t>((high_nibble << 4) | nibble));
      high_nibble = -1;
    }
  }
  if (payload.empty() || high_nibble >= 0) {
    ERROR("Preloaded SIB2 payload must contain an even, nonzero number of hex digits");
    return false;
  }

  asn1::rrc::bcch_dl_sch_msg_s message;
  asn1::cbit_ref bits(payload.data(), payload.size());
  if (message.unpack(bits) != asn1::SRSASN_SUCCESS ||
      message.msg.c1().type() != asn1::rrc::bcch_dl_sch_msg_type_c::c1_c_::types::sys_info) {
    ERROR("Could not decode preloaded BCCH-DL-SCH SystemInformation payload");
    return false;
  }

  auto& sib_list = message.msg.c1().sys_info().crit_exts.sys_info_r8().sib_type_and_info;
  for (uint32_t index = 0; index < sib_list.size(); ++index) {
    if (sib_list[index].type().value == asn1::rrc::sib_info_item_c::types::sib2) {
      asn1::rrc::sib_type2_s sib2 = sib_list[index].sib2();
      ulsche.set_SIB2(&sib2);
      ulsche.set_config();
      std::cout << "[CONFIG] Preloaded SIB2 from " << payload.size()
                << "-byte BCCH-DL-SCH payload; UL decoding starts at the first subframe" << std::endl;
      return true;
    }
  }
  ERROR("Preloaded SystemInformation payload does not contain SIB2");
  return false;
}
  ...
  api_mode(args.api_mode)
{
  ...
  if (!args.sib2_payload_hex.empty() && !preload_sib2_payload(args.sib2_payload_hex, ulsche)) {
    throw std::runtime_error("Invalid preloaded SIB2 payload");
  }
  /*Create PHY*/
...
bool LTESniffer_Core::run(){
  ...
  if (args.input_file_name != "") {
    ...
    cell.cp              = static_cast<srsran_cp_t>(args.file_cp);
    cell.phich_length    = static_cast<srsran_phich_length_t>(args.file_phich_length);
    cell.phich_resources = static_cast<srsran_phich_r_t>(args.file_phich_resources);
    ...
    // srsRAN enables wrapping by default for file-backed UE sync. Offline
    // captures must stop at EOF unless the user explicitly requested -w.
  ...
  // All queued subframes must finish before their shared statistics and PCAP
  // state are read or destroyed.
  phy->joinPending();
